...
/**
 * `/mypage/bookmarks` のエラー境界。`getMyBookmarks` が DB 例外を throw した場合などにここで捕捉する。
 * Next.js の error.tsx は Client Component 必須。
 */
export default function MyBookmarksError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[/mypage/bookmarks] error boundary caught', error);
  }, [error]);

  return (
    <main className="mx-auto max-w-2xl px-6 pb-24 pt-16 text-center">
      <h1 className="font-serif text-2xl font-bold tracking-tight md:text-3xl">
        {COPY.error.bookmarkList.title}
      </h1>
      <p className="mt-4 text-sm leading-7 text-ink-muted">{COPY.error.bookmarkList.description}</p>
