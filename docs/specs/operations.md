...
| 項目                 | 内容                                                                                                                                                       |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **設置箇所**         | AI 判定結果（楽天ブックス）/ 花の種類詳細（楽天市場）/ スポット詳細（楽天トラベル）の 3 箇所のみ。一覧ページ・トップには載せない                           |
| **無料枠の制約**     | 楽天ウェブサービスは無料。Application ID 単位で **1 秒 1 リクエスト** / **1 日上限あり**。`fetch` の `next.revalidate` 経由で叩く設計が必須                |
| **キャッシュ戦略**   | books: 24h / products: 12h / hotels: 1h。失敗時 / 空時はサイト内でフォールバック（楽天検索ページへのテキストリンク）に切り替え                             |
| **法令対応**         | 景品表示法ステマ規制（2023 年 10 月施行）対応として、全アフィリエイトリンクに「広告」バッジ + `rel="sponsored"` を付与。プライバシーポリシーに条項追加済   |
| **レポート確認**     | [楽天アフィリエイト管理画面](https://affiliate.rakuten.co.jp/) > レポート。日次・月次のクリック数・確定報酬を確認                                          |
| **報酬受取設定**     | 楽天キャッシュ受取（手数料無料・¥1 から）を推奨。銀行振込は ¥3,000 から                                                                                    |
| **環境変数（任意）** | `RAKUTEN_APPLICATION_ID` / `RAKUTEN_ACCESS_KEY` / `RAKUTEN_AFFILIATE_ID`。2026-05-14 API 移行で `accessKey` が必須化。未設定なら広告枠は静かに非表示になる |

詳細は `docs/22a_rakuten-affiliate.md`。
