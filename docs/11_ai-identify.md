...
- [ ] チケット [08](./08_flower-pages.md) で決めたホスティング方針に従い 1〜2 種（例：桜・ひまわり）の URL を `images` に手動 INSERT（`supabase/seed.sql` 末尾のテンプレート参照）
- [ ] 判定結果 UI で実画像とプレースホルダーが混在しても破綻しないことを確認
