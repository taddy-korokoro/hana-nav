...
- [ ] 花マスターの代表画像を `images` に登録（owner_type='flower'）　※画像 URL を curate 後、`seed.sql` 末尾のテンプレート参照
...
- セキュリティアドバイザー指摘事項（後続マイグレーションで対応）：
  - 自作関数 5 つ（`set_updated_at` / `handle_new_user` / `validate_image_owner` / `cascade_soft_delete_*`）に `SET search_path = ''` を追加。
  - `handle_new_user`（SECURITY DEFINER）の `EXECUTE` 権限を `anon` / `authenticated` から REVOKE。
  - PostGIS 拡張を `extensions` スキーマへ移動（Supabase 推奨）。
- 花マスター代表画像の `images` 投入は、画像 URL を curate 後に `seed.sql` 末尾テンプレートで実施。
