...
- 作業は **タスク単位**（T01〜T13）で実施。Claude への依頼時は必ずタスク ID（例：「T03 を進めて」）を含める
...
### T08: 詳細設計サンプル - API: AI 花判定 [model: sonnet]

- 出力: `docs/design-docs/08_detail-api-ai-identify.md`
- 工数目安: 0.5〜1 人日
- 含める内容:
  - リクエスト・レスポンスのスキーマ
  - エラーレスポンス
  - 認可
  - レート制限ロジック（匿名 1/日 / ログイン 3/日）

### T09: AI 花判定処理フロー詳細 [model: opus]

- 出力: `docs/design-docs/09_ai-flow.md`
- 工数目安: 1〜1.5 人日
- 含める内容:
...
### T10: ADR-001 論理削除方針 [model: opus]

- 出力: `docs/design-docs/adr/001-soft-delete.md`
- 工数目安: 0.5〜1 人日
- 含める内容: 全テーブル `deleted_at` 必須・カスケード論理削除トリガー・レビューの退会後扱い

### T11: ADR-002 AI 花判定の 3 段階フォールバック [model: opus]

- 出力: `docs/design-docs/adr/002-ai-fallback.md`
- 工数目安: 0.5〜1 人日
- 含める内容: なぜ 3 段階か / 各段階の役割 / コストとのバランス

### T12: ADR-003 Instant Navigation 採用 [model: opus]

- 出力: `docs/design-docs/adr/003-instant-navigation.md`
- 工数目安: 0.5〜1 人日

### T13: README 整備 [model: sonnet]
