...
- **Next.js 16 App Router**（React 19 + Cache Components）を Server Components 前提で組み、SSR / CSR の境界を末端まで押し下げ
- **Supabase（PostgreSQL + RLS + Storage + Auth）** を `@supabase/ssr` でラップし、Cookie 同期と認可をミドルウェア層に集約
- **Gemini API** での花判定を、「総称（マッチング用）」と「品種名（表示用）」の分離 + 3 段階フォールバックで再現率を確保
- **管理画面・データ収集パイプライン・運用ガード**まで実装し、本番運用に必要な保守動線まで一通り通している
...
「なぜそう書いてあるか」を 1 文で答えられる粒度に揃え、根拠を `CLAUDE.md`（規約）と `docs/specs/`（仕様）に分割して文書化している。

### 1. Server Components 前提の徹底（App Router）

> データ取得・状態管理・Mutation の第一選択を Server 側に倒し、Client 境界を末端まで押し下げる。

- データ取得は Server Component 内で直接 `async/await`。`useEffect + fetch` をクライアントから走らせない
- `'use client'` は **末端の葉**まで押し下げ、一覧ページ全体には付けない
- Mutation は **Server Actions** が第一選択、外部から叩く REST 的なものだけ Route Handler
- 検索フィルタなど共有・SEO・戻るボタン要件のある状態は **`searchParams` に持たせて Server 側で読む**
- 失敗パターンは `CLAUDE.md` の「やりがちな NG パターン」として固定化

### 2. Cache Components による read-your-own-writes

> Admin 編集 → 一覧画面への即時反映を、Next.js 16 の Cache Components で実現する。

`'use cache'` + `cacheTag()` で公開ページのキャッシュキーをエンティティ単位に持たせ、Admin の Server Action では `updateTag()` で同期的に該当タグを破棄する。
...
`revalidateTag` ではなく `updateTag` を選んだのは、admin 編集直後の閲覧で **即時反映** を保証したかったため（前者は次リクエストで lazy 再検証）。タグ定数は `lib/cacheTags.ts` に集約し、文字列直書きを禁止して invalidation 漏れを防ぐ。

### 3. AI 判定の信頼性とコスト

> 再現率とコスト爆発の両方を、データ設計とレート制限の組み合わせで抑える。

**再現率対策**

- Gemini に「総称（マッチング用 `flower_name`）」と「品種名（表示用 `cultivar`）」を両方返させる
- マッチングは `flowers.name` → `flower_aliases.alias` の順で **3 段階フォールバック**
- マスター未登録時は「自信なし」と明示し、誤った関連スポットを出さない

**コスト爆発対策**

- クライアント側で **max 1024px / JPEG 0.8 / 2MB 以下**にリサイズしてから送信
- **SHA-256 ハッシュをキー**にクライアント側で 24 時間キャッシュし、同一画像の重複呼び出しをゼロ化
